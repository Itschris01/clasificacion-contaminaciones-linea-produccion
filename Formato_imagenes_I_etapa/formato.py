...
# Carpetas de entrada y salida para cada clase del dataset.
...
def procesar_imagen(ruta_imagen, carpeta_salida, etiqueta):
    try:
        with Image.open(ruta_imagen) as img:
            # Se unifica formato y tamano antes de generar las variantes.
            img = img.convert("RGB")
            img = img.resize(TAMANO, Image.Resampling.LANCZOS)

            nombre_base = os.path.splitext(os.path.basename(ruta_imagen))[0]

            # Esta imagen RGB reducida sirve como referencia visual del preprocesado.
            ...
            # El umbral adaptativo ayuda cuando la iluminacion cambia entre fotos.
            ...
            # El CSV final guarda cada imagen como un vector binario mas su etiqueta.
...
def guardar_csv(datos, ruta_csv):
    # Cada pixel ocupa una columna y al final se agrega la clase esperada.
...
def main():
    # Se procesa cada carpeta por separado para conservar su etiqueta.
